//! Global state management for the platform bridges
//!
//! Uses OnceLock for thread-safe singleton pattern, similar to WASM implementation.
//!
//! Every global here is platform-neutral. Platform-specific handles (the
//! `JavaVM` and the Java status callback) stay in `jni_bridge::types`.
...
/// Platform-neutral status listener, registered by `lifecycle::init`.
///
/// The Android bridge registers a listener that forwards to the Java status
/// callback; the UniFFI bridge registers one that forwards to the Swift
/// `StatusListener`. Both receive the raw `STATE_*` value.
pub static STATUS_LISTENER: OnceLock<Box<dyn Fn(u8) + Send + Sync>> = OnceLock::new();
...
/// Invoke the registered status listener, if any.
///
/// Never fails: a bridge that did not register a listener simply gets no
/// notifications.
pub fn notify_status(status: u8) {
